...
global
    ...
    log 127.0.0.1 local0 info      #全局的日志配置，level为 err/warning/info/debug日志记录级别,log $host_ip local0 $level
...
frontend web_server                 #定义前端名称，可自定义
    ...
    default_backend             app                                                     #默认使用名称为app的backend后端
```
...
backend web                             #定义后端名称，自定义，要与前端指定的后端名一致
    ...
    option httpchk GET /1b.html HTTP/1·0#监控检测的方法，详见下面的\<option httpchk\>
