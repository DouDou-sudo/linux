...
storage不是namespace作用域的对象，pv也不是namespace作用域的对象但是它会和namespace作用域的pvc进行绑定
...
volumeBindingMode指定了卷绑定和动态制备发生在什么时候
  ...
  WaitForFirstConsumer 延迟pvc和pv的创建和绑定，直到pod的创建。只有Local 插件支持，如果pod使用此策略的storage，不能使用节点亲和性会导致pvc pending。可以使用nodeSelecter
```
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
...
##### 6、默认storageClass
设置默认的storageClass，将storageclass.kubernetes.io/is-default-class设置为true
