...
查看更改后的 /etc/containerd/config.toml
```
disabled_plugins = []
[grpc]
...
```
###### 2）直接加入
如果token过期，执行命令重新生成`kubeadm token create --print-join-command`
如果master初始化时加上了`--upload-certs`参数,可以直接添加--certificate-key \[certificate key\]加入control-plan节点到集群，不需要拷贝相关ca证书；
如果没有加上`--upload-certs`参数可以执行3，4步，也可以重新上传证书并生成新的解密密钥，然后在直接使用--certificate-key加入control-plan节点到集群。
